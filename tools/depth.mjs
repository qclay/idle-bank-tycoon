...
// ── Теней под предметом ровно одна ───────────────────────────────────────────

const shadows = await p.evaluate(() => {
  const { scene } = window.__game;
  const B = window.__balance;
  const v = window.__views.counters.get('c1');
  return {
    наПолу: scene.floorShadowCount(),
    ожидалось: B.WALLS.length + 1,      // стены и касса, витрины сюда не входят
    // имена классов после сборки минифицируются, поэтому тень помечена меткой
    вВитрине: (v?.children || []).filter((c) => c.label === 'shadow').length,
  };
});
ok('на полу тени только от стен и кассы', shadows.наПолу === shadows.ожидалось,
   `${shadows.наПолу} при ожидаемых ${shadows.ожидалось}`);
ok('под витриной ровно одна собственная тень', shadows.вВитрине === 1,
   `${shadows.вВитрине} шт.`);
