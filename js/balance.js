...
export const ROOMS = [
  ...
  { id: 'cash',    name: 'Касса',        x0: 0,  y0: 0, x1: 4,  y1: 4,  floor: 0xE7D9B8 },
  { id: 'lockers', name: 'Постаматы',    x0: 0,  y0: 4, x1: 4,  y1: 15, floor: 0xCBD9EC },
  { id: 'pickup',  name: 'Пункт выдачи', x0: 4,  y0: 0, x1: 20, y1: 6,  floor: 0xE4E8F4 },
  { id: 'sales',   name: 'Торговый зал', x0: 4,  y0: 6, x1: 20, y1: 15, floor: 0xEFEFEF },
  { id: 'service', name: 'Сервис',       x0: 20, y0: 0, x1: 24, y1: 15, floor: 0xF0DCE4 },
  { id: 'stock',   name: 'Склад',        x0: 24, y0: 0, x1: 28, y1: 15, floor: 0xC9BFA8, dark: true },
];
...
// ─── Первые минуты ───────────────────────────────────────────────────────────
// Новый игрок видит утку, джойстик и десяток ценников — и не понимает ничего.
// Поэтому первые шаги игра ведёт за руку: по одному делу за раз, тем же
// указателем, что и обычно, — ничего нового учить не нужно.

export const TUTOR = [
  { id: 'serve',  text: 'Встаньте за стойку и обслужите клиента' },
  { id: 'pick',   text: 'Заберите выручку с прилавка' },
  { id: 'drop',   text: 'Отнесите деньги в кассу' },
  { id: 'buy',    text: 'Встаньте на площадку — откроется вторая витрина' },
  { id: 'hire',   text: 'Наймите оператора: он будет работать за вас' },
];
...
export const REP = {
  ...
  // Претензия — это событие, а не фон. Раньше недовольным становился каждый
  // двадцатый клиент, и при полусотне человек в минуту игра превращалась в
  // погоню за настроением. Теперь это редкий случай, и между двумя разборами
  // обязательно проходит время.
  upsetBase: 0.012,          // базовый шанс, что клиент останется недоволен
  upsetPerWait: 0.4,         // + за долю исчерпанного терпения в очереди
  upsetMoraleWeight: 0.25,   // + если у оператора просела мораль
  upsetMax: 0.22,            // выше этого шанс не поднимается
  upsetGap: 150,             // и не чаще одного разбора в эти секунды
  waitTimeout: 24,           // сколько секунд клиент ждёт разбора
  moodAt: 0.5,               // доля терпения, после которой клиент мрачнеет
  angryAt: 0.82,             // а после которой уже откровенно злится
...
export const DISTRICT = {
  ...
  // Соперник появляется не сразу. Пока игрок разбирается со своим магазином,
  // гонка за район только мешает: это ещё одна цифра, которую он не понимает.
  // Дом напротив до этого стоит пустой, за строительным забором.
  needLevel: 15,
